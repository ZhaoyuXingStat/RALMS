function result = CRALMS(y, Phi, varargin)
% CRALMS  Constrained Robust Adaptive Lasso with Multi-Signal Shrinkage
%
%   result = CRALMS(y, Phi)
%   result = CRALMS(y, Phi, 'tau', 0.5, 'lambda', 0.1, 'rho', 1.0, ...)
%
...
%     'lambda'   - regularization parameter (positive). If empty,
...
%     'max_iter' - maximum ADMM iterations, default 1000
%     'tol'      - convergence tolerance, default 1e-5
%     'verbose'  - logical, print progress, default false
%
%   Output:
%     result : struct with fields
%       .x_hat     - estimated signal (p x 1, values in [0,1])
%       .weights   - adaptive weights (p x 1)
%       .lambda    - lambda used
%       .tau       - quantile level
%       .rho       - ADMM parameter
%       .n_iter    - ADMM iterations used
%       .converged - logical
%
%   Reference:
%     [Your paper citation here]
    ...
    addParameter(p, 'rho', 1.0, @isnumeric);
    ...
    if length(y) ~= n
        error('CRALMS:dimMismatch', ...
              'Dimension mismatch: length(y)=%d but size(Phi,1)=%d.', length(y), n);
    end
    if n < 2, error('CRALMS:tooFewObs', 'Need at least 2 observations.'); end
    if p_dim < 1, error('CRALMS:noPredictors', 'Need at least 1 predictor.'); end
    if opts.tau <= 0 || opts.tau >= 1
        error('CRALMS:invalidTau', 'tau must be in (0, 1).');
    end
    if ~isempty(opts.lambda) && opts.lambda <= 0
        error('CRALMS:invalidLambda', 'lambda must be positive.');
    end
    if opts.rho <= 0
        error('CRALMS:invalidRho', 'rho must be positive.');
    end
    if opts.gamma_w < 0
        error('CRALMS:invalidGamma', 'gamma_w must be non-negative.');
    end
    if any(isnan(y)) || any(isnan(Phi(:)))
        error('CRALMS:nanInput', 'Input contains NaN values.');
    end
    if any(~isfinite(y)) || any(~isfinite(Phi(:)))
        error('CRALMS:nonFinite', 'Input contains non-finite values.');
    end
    ...
    lam = opts.lambda;
    if isempty(lam)
        if verbose, fprintf('CRALMS: Selecting lambda via BIC ...\n'); end
        grid = compute_lambda_grid_q(y, Phi, tau, 20, 1e-4);
        bics = zeros(length(grid), 1);
        for gi = 1:length(grid)
            [xh, ~, ~, ~] = cralms_solve(y, Phi, tau, grid(gi), rho_val, ...
    ...
    [x_hat, weights, n_iter, converged] = ...
        cralms_solve(y, Phi, tau, lam, rho_val, gamma_w, max_it, tol_val, n, p_dim);
...
function [x_hat, weights, n_iter, converged] = ...
        cralms_solve(y, Phi, tau, lam, rho, gamma_w, max_iter, tol, n, p)
    ...
    for k = 1:max_iter
        ...
        v_z = x_hat + u_hat / rho;
        for j = 1:p
            z_tilde = prox_multi_signal(v_z(j), lam * weights(j) / rho);
        ...
        % convergence check
        primal_res = norm(x_hat - z_hat);
        dual_res = rho * norm(z_hat - z_old);
        if primal_res < tol && dual_res < tol
            converged = true; n_iter = k; break;
        end
    end

    x_hat = z_hat;  % return z (the constrained variable)
end
...
function print_cralms_result(r)
    ...
    fprintf('  x_hat range            : [%.4f, %.4f]\n', min(r.x_hat), max(r.x_hat));
    if pp <= 30
        fprintf('  x_hat: ');
        fprintf('%.4f  ', r.x_hat);
        fprintf('\n');
